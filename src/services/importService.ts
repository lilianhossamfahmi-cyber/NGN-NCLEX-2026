...
export const parseJsonInput = (input: string): { success: boolean, data?: MasterQuestionItem[], error?: string } => {
    ...
    try {
        ...
        // We relaxed the check: as long as it's an object, we try to map it. 
        // The Import Mapper will assign a default 'unknown' type which later gets normalized by ItemRenderer.
        ...
        const mappedItems: MasterQuestionItem[] = validItems.map((raw: any) => {
            // Check if it's already a full MasterQuestionItem (has metadata)
            if (raw.metadata && raw.content) return raw as MasterQuestionItem;

            // Otherwise wrap raw content (External AI output)
            const mappedContent = {
                quickStart: { summary: raw.prompt || 'Imported Question' },
                clinicalData: raw.content?.clinicalData || null,
                structure: raw.content?.structure || raw
            };

            // [AUTO-REPAIR]: Fix incomplete Case Studies (Missing 6th Screen)
            // Ensure we are working with the correct structure reference
            const structureRef = mappedContent.structure || mappedContent;

            if (structureRef?.type === 'case-study' && Array.isArray(structureRef.screens)) {
                const screens = structureRef.screens;

                // [FIX]: Ensure mappedContent refers to the modified object
                mappedContent.structure = structureRef;

                if (screens.length > 0 && screens.length < 6) {
                    console.warn(`[Import-Repair] Case Study only has ${screens.length} screens. Appending placeholder.`);
                    const missingCount = 6 - screens.length;
                    for (let j = 0; j < missingCount; j++) {
                        screens.push({
                            type: 'multiple-response', // CHANGED to Multiple Response (SATA) as per user pref
                            id: `auto_repair_import_${j}`,
                            prompt: "⚠️ AI Generation Incomplete. This screen was added to protect the exam flow. (Import Fix) - Select Acknowledgement",
                            options: [
                                { id: 'opt1', text: "Acknowledgement (Correct)", isCorrect: true, rationale: "Generated by Auto-Repair System" },
                                { id: 'opt2', text: "Ignore", isCorrect: false, rationale: "Incorrect" }
                            ]
                        });
                    }
                }
            ...
            return {
                ...
                typeId: raw.type,
                metadata: {
                    title: raw.metadata?.title || `Imported ${raw.type}`,
                    authorId: 'External_Import',
                    createdAt: new Date().toISOString(),
                    updatedAt: new Date().toISOString(),
                    status: 'draft',
                    qualityScore: 80,
                    hasStudentPreview: true,
                    batchInfo: { batchId: 'import', batchSize: 1, temperature: 0, generationDate: new Date().toISOString() },
                    sourceOrigin: 'upload' as InputMode,
                    sourceReferences: []
                },
                pedagogy: {
                    difficultyLevel: 3,
                    clinicalFocus: 'Imported',
                    clinicalFocusTopics: []
                },
                aiSafetyChecks: {
                    runId: 'import',
