...
- **[SCORING_GOLD_STANDARDS.md](./SCORING_GOLD_STANDARDS.md)**: Official NCLEX-NGN scoring rules by item type.
- **[BRAIN_GOLD_STANDARDS.md](./BRAIN_GOLD_STANDARDS.md)**: Expert Dashboard design specifications.

---

## 6. Focus Monitor & Stress Metrics (System-Wide Runtime Standard)

**Goal:** Measure student 'Indecision' and 'Stress' using strictly objective interaction data.

### 6.1 The Metric: Interaction Ratio
Stress is calculated **per-item** (resetting on each new question screen).

**Formula:** `Ratio = Answer Reversals / Total Option Base`

*   **Answer Reversals (Numerator):**
    *   **Single Choice:** Changing an existing selection (A → B). First selection does NOT count.
    *   **Multi-Select:** Unchecking an option (Retraction).
*   **Total Option Base (Denominator):**
    *   **Standard Items:** `options.length`
    *   **Bow-Tie:** Actions + Conditions + Parameters pool sizes
    *   **Matrix:** Rows × Columns
    *   **Highlight:** Count of `<span id='hX'>` tokens

### 6.2 The Logic Table (Status Spectrum)
| Ratio | Status | Color | Animation Speed | Meaning |
| :--- | :--- | :--- | :--- | :--- |
| **≤ 1.0x** | **Decisive** | Green (#34d399) | 2.0s | Confident execution |
| **1.0x - 2.0x** | **Deliberate** | Teal (#2dd4bf) | 1.5s | Careful consideration |
| **2.0x - 3.0x** | **Hesitant** | Yellow (#eab308) | 0.9s | Second-guessing |
| **3.0x - 4.0x** | **Scattered** | Orange (#f97316) | 0.6s | Loss of focus |
| **> 4.0x** | **Panic** | Red (#f43f5e) | 0.3s | Random/Frantic inputs |
